...
### 1. `TeamCity` 서버 접속
![TeamCity First Start](/images/post/infra/teamcity/teamcity-in-docker/001.png)
* `https://your.teamcity.domain`로 접속한다.
   * `your.teamcity.domain`은 `TeamCity` 서버의 도메인 주소를 입력한다.
* `Proceed`를 클릭하여 다음으로 이동한다. 기존 데이터를 복구 할 때는 `Restore from backup`를 클릭하여 복구를 진행한다.

### 2. DB 연결 설정
![Database connection setup](/images/post/infra/teamcity/teamcity-in-docker/002.png)
...
### 4. 관리자 계정 생성
![Create administrator account](/images/post/infra/teamcity/teamcity-in-docker/003.png)
...
### 1. `Agent` 설정
![TeamCity Agent](/images/post/infra/teamcity/teamcity-in-docker/004.png)
...
![Authorize Agent-001](/images/post/infra/teamcity/teamcity-in-docker/005.png)
1. `Authorize...` 버튼을 클릭하여 `Authorize Agent-001` 다이얼로그를 표시한다.
2. 필요한 경우 `Agent`에 대한 설명을 기입한다.
3. `Authorize` 버튼을 클릭하여 `Agent`를 인증한다.

### 2. 도메인 설정
![TeamCity Administration](/images/post/infra/teamcity/teamcity-in-docker/006.png)
...
### 3. Email 알림 설정
![TeamCity Administration](/images/post/infra/teamcity/teamcity-in-docker/007.png)
